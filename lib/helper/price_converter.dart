...
class PriceConverter {
  static String convertPrice(double? price, {double? discount, String? discountType, bool forDM = false}) {
    ...
    return '${isRightSide ? '' : '${Get.find<LocationController>().getUserAddress()!.zoneData![0].zoneCurrency!} '}'
        '${(toFixed(price!)).toStringAsFixed(forDM ? 0 : Get.find<SplashController>().configModel!.digitAfterDecimalPoint!)
        .replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
        '${isRightSide ? ' ${Get.find<LocationController>().getUserAddress()!.zoneData![0].zoneCurrency!}' : ''}';
  }
  ...
  static Widget convertAnimationPrice(double? price, {double? discount, String? discountType, bool forDM = false, TextStyle? textStyle}) {
    ...
    return Directionality(
      ...
      child: AnimatedFlipCounter(
        ...
        prefix: isRightSide ? '' : Get.find<LocationController>().getUserAddress()!.zoneData![0].zoneCurrency!,
        suffix: isRightSide ? Get.find<LocationController>().getUserAddress()!.zoneData![0].zoneCurrency! : '',
      ),
    );
  }
  ...
  static String percentageCalculation(String price, String discount, String discountType) {
    return '$discount${discountType == 'percent' ? '%' : Get.find<LocationController>().getUserAddress()!.zoneData![0].zoneCurrency!} OFF';
  }

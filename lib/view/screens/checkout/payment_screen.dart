...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<bool?> _exitApp() async {
    if (kDebugMode) {
      print('----------test widget.addFundUrl: ${widget.addFundUrl} / widget.addFundUrl!.isEmpty ${widget.subscriptionUrl}/ ${widget.orderModel.id}');
    }
    if((widget.addFundUrl == null || widget.addFundUrl!.isEmpty) && (widget.subscriptionUrl == null || widget.subscriptionUrl!.isEmpty)){
    ...
    if(_canRedirect) {
      ...
      if(((addFundUrl == null || addFundUrl!.isEmpty) && (subscriptionUrl == null || subscriptionUrl!.isEmpty))){
        ...
        if(isSuccess || isFailed || isCancel) {
          ...
          if((subscriptionUrl != null && subscriptionUrl!.isNotEmpty) && (addFundUrl == null || addFundUrl!.isEmpty)) {
            if (kDebugMode) {
              print('---here--- 3  now');
            }

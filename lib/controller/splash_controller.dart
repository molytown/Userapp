...
class SplashController extends GetxController implements GetxService {
  ...
  void updateActivePaymentMethodList() {
    List<DigitalPayment>? zonePaymentList = Get.find<LocationController>().getUserAddress()?.zoneData?[0].digitalPayment;
    List<PaymentBody>? configPaymentList = _configModel!.activePaymentMethodList;
    List<PaymentBody>? activePaymentList = [];

    zonePaymentList?.forEach((element) {
      print('----------------------------> zonePaymentList ${element.title}, ${element.status}');
    });

    configPaymentList?.forEach((element) {
      print('----------------------------> configPaymentList ${element.getWay}');
    });

    if(zonePaymentList != null && configPaymentList != null) {
      for(int i = 0; i < zonePaymentList.length; i++) {
        for(int j = 0; j < configPaymentList.length; j++) {
          if(zonePaymentList[i].title == configPaymentList[j].getWay && zonePaymentList[i].status.toString() == '1') {
            activePaymentList.add(configPaymentList[j]);
          }
        }
      }
    }
    for (var element in activePaymentList) {
      print('----------------------------> activePaymentList ${element.getWay}');
    }

    _configModel!.activePaymentMethodList = activePaymentList;
  }
